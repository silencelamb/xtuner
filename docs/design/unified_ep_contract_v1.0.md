...
面向框架组与算子组同事的讲解稿。本文只讲最终态：代码在 [silencelamb/xtuner 分支 `feat/deepepv2`](https://github.com/silencelamb/xtuner/tree/feat/deepepv2)，基于 upstream `7d377424`，两个原地提交；本文与 [大 EP 整体设计 v0.2](large_ep_overall_design_v0.2.md)、[统一 Grouped GEMM 算子库 v1.0](unified_grouped_gemm_v1.0.md) 一起随第 1 个提交进入 `docs/design/`。讲解前的问答整理进 §2.6–2.8、§5.1–5.2、§6.1–6.2（2026-09-17）；§2.7 列出 v0.2 各定义在本文的归宿。算子侧配套：[统一 Grouped GEMM 算子库 v1.0](unified_grouped_gemm_v1.0.md)（§2.4 的 GEMM 入口在算子侧怎么扩成注册表与多后端，含开发指南）。

## 概要

| 内容 | 指向 |
|---|---|
| 核心在 `dispatcher/base.py` | §2 整节。数据面看 §2.1：`ExpertRows` 和 `ExpertWeights`，都挂在 `PostDispatchResult` 上；执行面看 §2.3：`LayerEPExecution` 五个钩子、`EPExecutionRuntime` 四个边界 |
...
- 行为面：legacy 路径的数值逐位不变（跨树对比：在上游原样树上 dump `MoEDecoderLayer` 的输出 / dX / 全部参数梯度，在新树上比）；速度不变（GLM-5.2 6347 vs 6261，Qwen3 mb1 1.798 s vs 1.788 s，噪声内）。最近一次完整回归（2026-09-19，代码内容与本分支 tip 相同、只差 `docs/design/` 下的文档）：三级逐位对比 66 条全部 BIT-EXACT，其中 AGRS 用分组路由器（16 专家、top-k 8）在 8 卡上跑；现有单测 14 通过 1 跳过；新增单测（契约、8 条 kernel 路径、DeepEP V2 六模式与 CUDA graph / compile）25 通过，单机无 GIN 网卡时要带 `EP_DISABLE_GIN=1`。
...
### 2.7 v0.2 里的布局、空间、均衡器定义去了哪里

v0.2 与它的两份扩展草案定义过一批类型，v1.0 落地后各有归宿：一部分改名吸收，一部分被钩子溶解，一部分推迟到 FSDP/EP 解耦那条线（§2.8）。

| v0.2 / 草案里的定义 | v1.0 里的归宿 | 结论 |
|---|---|---|
| `Layout` 枚举 R / E0 / EA / M，`layout: Literal["E0","EA"]` | `ExpertRows.alignment`：1 就是 E0，128 就是 EA。R 布局在 legacy dispatcher 的 `dispatch_postprocess` 内部 permute 掉，契约看不到；M 是 decode 专用，不在训练范围 | 不保留枚举。E0 / EA 作为口头术语留在文档里（`ExpertRows` docstring 已这么用）。将来 SonicMoE 那种直接吃 R 加 `gather_idx` 的 GEMM 进来时再加字段，按 §7 "等第一个需要它的后端出现再开" |
| `SFFormat` | `PostDispatchResult.hidden_scales` 加 bf16 类型的 `Float8Tensor` 包装；scale 排布是各 GEMM 后端内部的事 | 不保留 |
| `ExpertSpace(kind, global_size)`、`Routing.space` | 没有对应类型。逻辑 / 物理的区分靠三条约定：dispatcher 构造时只吃一个整数 `n_routed_experts`（#2050 的 `num_dispatch_experts = E + ep×R`）；`prepare_dispatch` 之后的 `topk_ids` 就是物理 id；`dispatch_preprocess(tokens_per_expert=)` 是 router 的逻辑计数，只给声明需要的 dispatcher（MoonEP planning） | 不保留类型，三条约定写进两个方法的 docstring。草案担心的"重路由后逻辑计数被误用"在现有后端不会发生，DeepEP 两代都不读 `tokens_per_expert`。值得加一个 `DispatcherCaps` 位（如 `accepts_physical_ids`），让 `ultraep_cfg` 配 `dispatcher="moonep"` 在配置期被拒绝，替代 #2050 硬编码的 `dispatcher == "deepep"` |
| `Balancer` Protocol、`BalancePlan` | 溶解进五个钩子和 `EPCall.plan`，映射见下表 | 不保留。"dispatcher 之前的均衡器插件"只贴合 UltraEP；MoonEP 的均衡融合在 dispatch 的 planning kernel 里拆不出来。钩子的抽象层次更低，两家都装得下 |
| `ExpertWeights`（v0.2 版：`w1w3 / w2 / seg_sizes / storage / grad_sinks / ready`） | `ExpertWeights(segments)`：`seg_sizes` 变成 `first_slot` 加张量形状；`grad_sinks` 变成 `GradBinding.buffer`；`ready` 不是字段，由 `prepare_experts` 钩子内部等事件；**`storage` 字段消失**（原因见 §2.8） | 已吸收 |
| `ExpertParamAllocator`（专家参数 allocator 草案） | 不在契约里 | 归入 FSDP/EP 解耦工作流的后半段，见 §2.8 |

`Balancer` 六个方法与钩子的对应：

| v0.2 `Balancer` | v1.0 落点 |
|---|---|
| `setup(spec, weights)` | `EPExecutionRuntime.bind_layer` 加 `install_after_fsdp` |
| `plan(route)` | `prepare_dispatch`：`update_placement` 加 `reroute` |
| `materialize(plan)` | `prepare_dispatch` 里发 `sync_weights`，`prepare_experts` 里填 `ExpertWeights` 段 |
| `restore_for_backward(plan)` | `ProjectionWeight.backward_read="restore_before_dgrad"`，由 GEMM 层在 dgrad 前触发 |
| `reduce_grads(plan)` | `attach_after_experts` 或 `attach_after_combine`，dW 落点用 `GradBinding(path="external")` |
| `join(token)` | `prepare_layer_inputs` 在层入口放 Join |
| `BalancePlan.max_recv_rows` | 归到通信层的 `DeepEPV2Config.capacity_factor`，不再由均衡器承诺 |

### 2.8 FSDP bf16 权重与固定地址：两个 PR 的解法与 allocator 的位置

**现状。** 上游 `7d377424` 还没有解耦：`moe.py:1302` 仍调用 `_replicate_other_params` 把 dense 参数按 EP 倍数复制，专家与 dense 共用一个 `fsdp_mesh`。`efsdp` 维度只存在于 MetaMoE 分支的 `DECOUPLE_EP_FSDP=1` 实现里。所以 v0.2 §2.10 建议的 `efsdp=1` 加 `dtensor_ep_only` 在当前树上不可用。
...
**`ExpertParamAllocator` 的位置。** 它不是契约 v0 的内容，而是"解耦 FSDP 与 EP"那条线的后半段。要纠正 v0.2 一处说法：即使做到 `efsdp=1`，FSDP2 在 world size 为 1 时仍走 `_fsdp_param_group.py:357` 的路径，把 sharded 参数拷贝进一份 unsharded 输出，地址仍归 FSDP 管。要真正得到"分配一次、终身固定"的地址，专家模块必须**不进 `fully_shard`**，用 `DTensor.from_local(Shard(ep))` 保住 checkpoint 语义；这意味着专家的 fp32 master、optimizer state、梯度归约都要走单独路径（MetaMoE 分支的 `_scale_and_reduce_grad_decoupled`）。前置是三步：解耦 mesh 上游化；`efsdp=1` 成为合法配置且专家跳过 `fully_shard`；然后才有 allocator 挂接点。它值得保留，但应从 v0.2 的契约章节挪到解耦提案里作为 M3 阶段目标，收益写明：替换掉 MoonEP 那个钉 torch 版本的 FSDP 私有方法覆盖。`efsdp=1` 的显存代价（专家 fp32 master 与 Adam 不再按 efsdp 切分）对大 EP 可忽略，对单节点大 E 不行，v0.2 §2.10 的判断不变。
...
彻底去 NVSHMEM 有一条比等上游快的路（v0.2 设计文档 §3.4）：副本按构造永不出 NVLink 域（`nvshmem_ptr` 对域外 PE 返回空），对称堆可换成域内 IPC（`cuMemCreate` + fd 导出）或直接复用 NCCL 的 LSA 窗口；跨节点只剩 KB 级 all-gather，走 NCCL 的开关已有。主要改动在 `csrc/ultra_ep.cpp` 的堆分配与 peer 指针获取，是算子组体量的工作。
