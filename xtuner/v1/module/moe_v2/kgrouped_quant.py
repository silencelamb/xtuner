...
@triton_op("moe_v2::trans_quant_kgrouped", mutates_args={})
...
def static_ks_host(k_alloc: int, num_groups: int) -> list[int]:
    ...
    descriptors and picks a kernel config, so any 128-multiple list summing to the allocated K works. This keeps the
    call free of host syncs.

    Args:
        k_alloc (int): Allocated K (``scales.shape[1] * 128``).
        num_groups (int): Number of groups (local experts).

    Returns:
        list[int]: ``[k_alloc, 0, ..., 0]`` of length ``num_groups``.
    """
    return [k_alloc] + [0] * (num_groups - 1)
