...
def use_deepgemm(rows: "ExpertRows") -> bool:
    """Whether the expert GEMMs of a batch run on DeepGEMM's psum kernels.

    DeepGEMM's psum layout needs the ``EA`` layout aligned to exactly 128 rows (its ``BLOCK_M``), which is what
    DeepEP V2 produces with ``expert_alignment=128``; ``EA`` with another alignment or ``E0`` stays on the
    counts-based legacy kernels.
    ...
    if rows.alignment != 128:
        if backend == "deepgemm":
            raise ValueError(
                "XTUNER_EXPERT_GEMM_BACKEND=deepgemm needs the EA layout aligned to 128 rows (DeepEP V2 with "
                f"expert_alignment=128); got layout {rows.layout.name} with alignment {rows.alignment}"
            )
        return False
