...
def dispatch_forward(
    ...
    *,
    ...
    previous_event: Any,
    async_finish: bool,
    num_max_expanded_tokens: int | None = None,
    overflow_flag: Tensor | None = None,
    ...
    ``num_max_expanded_tokens`` caps the receive rows in the static (``do_cpu_sync=False``) mode below the worst
    case; rows beyond it are dropped and ``overflow_flag`` (CUDA int32) is set to 1 (patched DeepEP V2).
    """
    capped: dict[str, Any] = {}
    if num_max_expanded_tokens is not None:
        # Only the capped-capacity DeepEP build knows these keywords; an unpatched build keeps the worst case.
        capped = {"num_max_expanded_tokens": num_max_expanded_tokens, "overflow_flag": overflow_flag}
