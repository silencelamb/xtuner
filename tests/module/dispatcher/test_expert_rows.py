...
class TestExpertRows:
    def test_rows_from_counts_is_unpadded(self):
        ...
        assert rows.layout is Layout.E0
    ...
    def test_rows_from_psum_matches_expanded_aligned_layout(self):
        ...
        assert rows.layout is Layout.EA
    ...
    def test_layout_enum_is_stable(self):
        assert Layout("expert_contig") is Layout.E0 and Layout("expert_aligned") is Layout.EA
        assert {m.name for m in Layout} == {"E0", "EA", "R", "M"}

    def test_dispatcher_caps_derive_layout(self):
        assert DispatcherCaps(name="legacy").produces_layout is Layout.E0
        assert DispatcherCaps(name="v2", produces_alignment=128).produces_layout is Layout.EA
