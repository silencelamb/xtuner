"""SFT benchmark config for the MoE V2 (unified EP contract) path vs the legacy dispatchers.
...
Dispatcher switch: ``DISPATCHER=all2all|deepep|deepep_v2``. MoE V2 knobs (only read for ``deepep_v2``):
``MOE_V2_GEMM=auto|triton|adaptive_gemm|deepgemm``, ``MOE_V2_ALIGN=1|128``, ``MOE_V2_CPU_SYNC=0|1``,
``MOE_V2_FP8_DISPATCH=0|1``, ``MOE_V2_DETERMINISTIC=0|1``, ``MOE_V2_NUM_SMS=<int>``.
"""
...
work_dir = os.environ.get("WORK_DIR", "work_dirs/moe_v2_bench")
...
if dispatcher == "deepep_v2":
    model_cfg.moe_v2_cfg = MoEV2Config(
        dispatcher="deepep_v2",
        gemm_backend=os.environ.get("MOE_V2_GEMM", "auto"),  # type: ignore[arg-type]
        expert_alignment=int(os.environ.get("MOE_V2_ALIGN", "128")),
        max_tokens_per_rank=pack_max_length // sp_size,
        cpu_sync=_bool("MOE_V2_CPU_SYNC", True),
        fp8_dispatch=_bool("MOE_V2_FP8_DISPATCH", False),
        deterministic=_bool("MOE_V2_DETERMINISTIC", False),
        num_sms=int(os.environ.get("MOE_V2_NUM_SMS", "0")),
    )
...
dataset_config = [
    {
        "dataset": DatasetConfig(
            ...
            cache_tag=os.environ.get(
                "CACHE_TAG", f"moe_v2_{os.environ.get('CHAT_TEMPLATE', 'qwen3')}_{sample_max_length}"
            ),
        ),
